...
pub fn award_xp_system(mut events: EventReader<ExperienceGainedEvent>, mut state: ResMut<LevelProgress>) {
    // System award_xp_system: substrate-expanded body
    ...
    let _ = _events_last;
    // LevelProgress can be mutated below; we touch it as an audit hook
    let _ = &mut *state;
    if activity > 0 {
        // award_xp_system: tick had {activity} actionable events
    }
    let _ = activity;
}
