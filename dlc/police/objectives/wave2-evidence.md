# Worker: Evidence Domain (Wave 2)

## Scope (mechanically enforced — edits outside will be reverted)
You may only create/modify files under: `dlc/police/src/domains/evidence/`

## Required reading (read BEFORE writing code)
1. `dlc/police/docs/spec.md` — full game spec
2. `dlc/police/docs/domains/evidence.md` — YOUR domain spec (CRITICAL — read every line, especially the enumerative data tables)
...
## Deliverables
Create `dlc/police/src/domains/evidence/mod.rs` with:
- `pub struct EvidencePlugin;` implementing `Plugin`
