...
use super::maps::{ObjectPlacement, WorldObjectKind};
use super::WorldMap;
...
pub struct FurnitureAtlases {
    ...
    /// Animated processing machine sprite sheet (1984x96, 64x48 tiles, 31 cols x 2 rows).
    /// Row 0 contains the processing animation; row 1 has overlay/particle effects.
    pub machine_anim_image: Handle<Image>,
    pub machine_anim_layout: Handle<TextureAtlasLayout>,
...
pub fn ensure_furniture_atlases_loaded(
    ...
    // Animated machine sprite sheet: 1984x96, 64x48 tiles → 31 cols × 2 rows
    atlases.machine_anim_image = asset_server.load("sprites/machine_anim.png");
    atlases.machine_anim_layout = layouts.add(TextureAtlasLayout::from_grid(
        UVec2::new(64, 48),
        31,
        2,
        None,
    ...
    pub fn drops(self) -> Vec<(&'static str, u8)> {
        match self {
            WorldObjectKind::Tree => {
                vec![("wood", 4), ("sap", 2), ("hardwood", 1), ("tree_seed", 1)]
            }
    ...
    for placement in placements {
        ...
        if object_atlases.loaded {
            ...
            let (image, layout, index) = if is_tree {
                ...
                let row_offset = if matches!(kind, WorldObjectKind::Pine) {
                    4
                } else {
                    0
                };
                ...
                if event.tool == effective {
                    ...
                    if tier_level(event.tier) < tier_level(required) {
                        toast_writer.send(ToastEvent {
                            message: format!("Need a better {}!", tool_display_name(event.tool)),
                            duration_secs: 2.0,
                        });
                        break;
                    }
                    ...
                    if new_health == 0 {
                        ...
                        if matches!(obj_data.kind, WorldObjectKind::Tree | WorldObjectKind::Pine) {
                            ...
                            if object_atlases.loaded {
                                ...
                                let stump_wc =
                                    grid_to_world_center(obj_data.grid_x, obj_data.grid_y);
                                commands.spawn((
                                    stump_sprite,
...
pub fn spawn_forageables(
    ...
    for (i, &(gx, gy)) in forage_points.iter().enumerate() {
        ...
        commands.spawn((
            sprite,
            Transform::from_translation(Vec3::new(fwc.x, fwc.y, Z_ENTITY_BASE)),
            WorldObject,
            YSorted,
            Forageable {
                item_id: item_id.to_string(),
...
pub fn spawn_daily_weeds(
    ...
    for event in day_events.read() {
        ...
        let weed_count = 2
            + ((event.day as usize)
                .wrapping_mul(13)
                .wrapping_add(event.season.index()))
                % 3;
        ...
        for attempt in 0..(weed_count * 10) {
            ...
            let y = (((hash / farm_w as usize)
                .wrapping_mul(17)
                .wrapping_add(attempt * 3))
                % farm_h as usize) as i32;
    ...
    for event in season_events.read() {
        ...
        for attempt in 0..(tree_count * 15) {
            ...
            if object_atlases.loaded {
                ...
                commands.spawn((
                    sprite,
                    Transform::from_translation(Vec3::new(rwc.x, rwc.y + y_offset, Z_ENTITY_BASE)),
                    WorldObject,
                    YSorted,
                    data,
                ));
...
fn resolve_building_image(img: BuildingImage, atlases: &ObjectAtlases) -> Handle<Image> {
    match img {
...
fn building_image_source_size(img: BuildingImage) -> Vec2 {
    match img {
...
pub fn spawn_building_sprites(
    ...
    for bld in &buildings {
        if let Some(img) = bld.composite {
            // ── Composite sprite path ──────────────────────────────────
            // Render the entire building (walls + roof) as a single image.
            // Width is set to match the footprint tile width; height is
            // derived from the source aspect ratio so the image is not
            // distorted.
            let src = building_image_source_size(img);
            let aspect = src.y / src.x;
            let render_w = bld.w as f32 * TILE_SIZE;
            let render_h = render_w * aspect;

            // Anchor the sprite so its horizontal center aligns with the
            // footprint center and its bottom edge sits at bld.y * TILE_SIZE.
            let center_x = bld.x as f32 * TILE_SIZE + render_w * 0.5 - TILE_SIZE * 0.5;
            let bottom_y = bld.y as f32 * TILE_SIZE - TILE_SIZE * 0.5;
            let center_y = bottom_y + render_h * 0.5;
            ...
            commands.spawn((
                ...
                WorldObject,
                sprite,
                Transform::from_xyz(center_x, center_y, Z_GROUND + 1.5),
                Visibility::default(),
            ));
        } else {
            // ── Legacy tile-by-tile path (Sprout Lands house tilesets) ──
