...
fn mayor_rex_schedule(season: Season) -> NpcSchedule {
    ...
    let (afternoon_map, afternoon_x, afternoon_y) = match season {
        ...
        Season::Winter => (MapId::Town, 35, 10),
    ...
    NpcSchedule {
        weekday: vec![
            ...
            ScheduleEntry { time: 12.0, map: MapId::Town, x: 20, y: 22 }, // lunch bench
...
fn margaret_schedule(season: Season) -> NpcSchedule {
    ...
    let (eve_map, eve_x, eve_y) = match season {
        ...
        Season::Winter => (MapId::Town, 35, 10),    // library warmth
        Season::Fall   => (MapId::Forest, 10, 15),  // admiring the foliage
        Season::Spring => (MapId::Town, 12, 30),    // south garden
    };
...
fn elena_schedule(season: Season) -> NpcSchedule {
    ...
    if evening_mine {
        ...
        weekday.push(ScheduleEntry { time: 21.0, map: MapId::Town, x: 34, y: 12 }); // home
    } else {
        weekday.push(ScheduleEntry { time: 19.0, map: MapId::Town, x: 34, y: 12 }); // home (winter, stays warm)
    }
    weekday.push(ScheduleEntry { time: 22.0, map: MapId::Town, x: 34, y: 12 }); // sleep
...
fn doc_schedule(season: Season) -> NpcSchedule {
    ...
    let (herb_map, herb_x, herb_y) = match season {
        ...
        Season::Fall   => (MapId::Forest, 10, 15),  // mushrooms and late-season herbs
        Season::Spring => (MapId::Forest, 8, 14),   // spring herbs
    };
...
fn old_tom_schedule(season: Season) -> NpcSchedule {
    ...
    let (primary_x, primary_y, secondary_x, secondary_y) = match season {
        ...
        Season::Winter => (14, 26, 12, 24), // huddled closer to shore
        Season::Fall   => (16, 28, 14, 26), // back from summer spots
        Season::Spring => (16, 26, 14, 24), // comfortable spots
    };
    ...
    let (noon_x, noon_y) = if matches!(season, Season::Winter) { (18, 14) } else { (16, 20) };
    ...
    NpcSchedule {
        weekday: vec![
            ...
            ScheduleEntry { time: 17.0, map: MapId::Beach, x: 16,          y: 24          }, // evening campfire area
        ...
        rain_override: Some(vec![
            ...
            ScheduleEntry { time: 12.0, map: MapId::Beach, x: 16,          y: 20          }, // still won't leave
...
fn marco_schedule(season: Season) -> NpcSchedule {
    ...
    let (ingredient_hunt_map, ing_x, ing_y) = match season {
        ...
        Season::Winter => (MapId::Town, 24, 30),    // indoor prep (no fresh sources)
        Season::Spring => (MapId::Farm, 20, 20),    // visits farm for spring produce
    };

    NpcSchedule {
        weekday: vec![
            ScheduleEntry { time: 5.0,  map: MapId::Town,                x: 26, y: 28 }, // home (early cook rise)
            ...
            ScheduleEntry { time: 10.0, map: MapId::Town,                x: 24, y: 32 }, // lunch service open
        ...
        rain_override: Some(vec![
            ScheduleEntry { time: 6.0,  map: MapId::Town, x: 26, y: 28 }, // home
            ScheduleEntry { time: 8.0,  map: MapId::Town, x: 24, y: 30 }, // restaurant all day (shelter draw)
...
fn sam_schedule(season: Season) -> NpcSchedule {
    ...
    let (weekend_pm_map, wknd_pm_x, wknd_pm_y) = match season {
        ...
        Season::Fall   => (MapId::Forest, 10, 15),
        _              => (MapId::MineEntrance, 6, 12),
    };

    NpcSchedule {
        weekday: vec![
            ScheduleEntry { time: 6.0,  map: MapId::Town,        x: 14, y: 36 }, // home
            ScheduleEntry { time: 7.0,  map: MapId::MineEntrance, x: 6,  y: 12 }, // on duty
            ScheduleEntry { time: 12.0, map: MapId::MineEntrance, x: 4,  y: 8  }, // lunch at entrance
            ScheduleEntry { time: 13.0, map: MapId::MineEntrance, x: 6,  y: 12 }, // back on post
        ...
        weekend: vec![
            ...
            ScheduleEntry { time: 9.0,  map: MapId::MineEntrance, x: 6,  y: 12 }, // quick check
            ScheduleEntry { time: 11.0, map: MapId::Town,        x: 26, y: 24 }, // south walk
        ...
        rain_override: Some(vec![
            ScheduleEntry { time: 6.0,  map: MapId::Town,        x: 14, y: 36 }, // home
            ScheduleEntry { time: 9.0,  map: MapId::MineEntrance, x: 6,  y: 12 }, // shelters at mine entrance
            ScheduleEntry { time: 19.0, map: MapId::Town,        x: 14, y: 36 }, // home
            ScheduleEntry { time: 21.0, map: MapId::Town,        x: 14, y: 36 }, // sleep
        ]),
        festival_override: Some(vec![
...
fn nora_schedule(season: Season) -> NpcSchedule {
    ...
    let (afternoon_outing_map, aft_x, aft_y) = match season {
        Season::Fall   => (MapId::Forest, 10, 15),
        Season::Summer => (MapId::Beach, 16, 20),   // cooling off
        _              => (MapId::Farm, 50, 52),     // still farming
    };

    NpcSchedule {
        weekday: vec![
            ScheduleEntry { time: 5.0,               map: MapId::Farm, x: 48, y: 50 }, // home (early riser)
            ScheduleEntry { time: farm_start_time,   map: MapId::Farm, x: 50, y: 52 }, // south plots
            ...
            ScheduleEntry { time: 18.0,              map: MapId::Town, x: 22, y: 38 }, // south town
        ...
        weekend: vec![
            ...
            ScheduleEntry { time: 7.0,  map: MapId::Farm, x: 46, y: 48 }, // inspecting player's area
            ScheduleEntry { time: 10.0, map: MapId::Town, x: 22, y: 38 }, // town market
        ...
        rain_override: Some(vec![
            ScheduleEntry { time: 5.0,  map: MapId::Farm, x: 48, y: 50 }, // home
            ScheduleEntry { time: 8.0,  map: MapId::Farm, x: 50, y: 52 }, // light rain work (rain is good!)
            ScheduleEntry { time: 12.0, map: MapId::Farm, x: 48, y: 50 }, // inside for heavy rain
...
fn lily_schedule(season: Season) -> NpcSchedule {
    ...
    let (mid_morning_map, mid_x, mid_y) = match season {
        ...
        Season::Spring => (MapId::Town, 8, 22),     // near library oak tree
    };

    let (afternoon_map, aft_x, aft_y) = match season {
        Season::Summer => (MapId::Beach, 20, 20),   // sand castles
        Season::Fall   => (MapId::Forest, 10, 12),  // deeper forest
        Season::Winter => (MapId::Town, 18, 30),    // south park (brief outdoor time)
        Season::Spring => (MapId::Town, 18, 30),    // south park
    };
    ...
    NpcSchedule {
        ...
        weekend: vec![
            ...
            ScheduleEntry { time: 21.0, map: MapId::Town,        x: 16, y: 28 }, // bedtime (non-negotiable)
        ],
        rain_override: Some(vec![
            ScheduleEntry { time: 7.0,  map: MapId::Town, x: 16, y: 28 }, // home (mom says no)
            ScheduleEntry { time: 9.0,  map: MapId::Town, x: 8,  y: 20 }, // library (forced)
            ScheduleEntry { time: 13.0, map: MapId::Town, x: 16, y: 28 }, // lunch
        ...
        festival_override: Some(vec![
            ...
            ScheduleEntry { time: 21.0, map: MapId::Town, x: 16, y: 28 }, // forced bedtime even on festival
        ]),
    }
}
