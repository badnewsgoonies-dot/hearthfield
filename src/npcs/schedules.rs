...
pub fn apply_enhanced_schedules(calendar: Res<Calendar>, mut npc_registry: ResMut<NpcRegistry>) {
    ...
    info!(
        "[schedules] Applied enhanced {:?} schedules for all NPCs.",
        season
    );
}
...
pub fn refresh_schedules_on_season_change(
    ...
    for ev in season_events.read() {
        ...
        info!(
            "[schedules] Refreshed NPC schedules for new season: {:?}",
            season
        );
    }
}
...
pub fn check_farm_visits(
    ...
    for npc_id in candidates {
        ...
        let npc_hash: u32 = npc_id.bytes().enumerate().fold(0u32, |acc, (i, b)| {
        ...
        if let Some(schedule) = npc_registry.schedules.get_mut(&npc_id) {
            ...
            info!(
                "[schedules] {} will visit the farm today (30% roll succeeded).",
                npc_id
            );
        }
    }
...
fn mayor_rex_schedule(season: Season) -> NpcSchedule {
    ...
    NpcSchedule {
        ...
        festival_override: Some(festival_override_for_season(
            season,
            MapId::Town,
            22,
            10,
...
fn doc_schedule(season: Season) -> NpcSchedule {
    let (home_map, home_x, home_y) = (MapId::TownHouseWest, 8, 2);
    ...
    NpcSchedule {
        ...
        weekend: vec![
            ...
            ScheduleEntry {
                time: 12.0,
                map: herb_map,
                x: herb_x.saturating_add(4),
                y: herb_y.saturating_add(6),
            }, // deeper
            ScheduleEntry {
                time: 14.0,
                map: MapId::Town,
        ...
        festival_override: Some(festival_override_for_season(
            season, home_map, home_x, home_y, 22, 18,
        )),
    }
}
...
fn old_tom_schedule(season: Season) -> NpcSchedule {
    let (home_map, home_x, home_y) = (MapId::TownHouseEast, 2, 7);
    ...
    let noon_map = if matches!(season, Season::Winter) {
        MapId::Town
    } else {
        MapId::Beach
    };
    let (noon_x, noon_y) = if matches!(season, Season::Winter) {
        (18, 14)
    } else {
        (16, 9)
    };
    ...
    NpcSchedule {
        ...
        festival_override: Some(festival_override_for_season(
            season, home_map, home_x, home_y, 18, 18,
        )),
    }
}
    ...
    NpcSchedule {
        weekday: vec![
            ScheduleEntry {
                ...
                x: 18,
                y: 17,
            }, // home (early cook rise)
            ScheduleEntry {
                time: 6.0,
                map: ingredient_hunt_map,
                x: ing_x,
                y: ing_y,
            }, // seasonal ingredient run
            ScheduleEntry {
                time: 8.0,
                map: MapId::Town,
                x: 16,
        ...
        rain_override: Some(vec![
            ...
            ScheduleEntry {
                ...
                x: 16,
                y: 16,
            }, // restaurant all day (shelter draw)
            ScheduleEntry {
                time: 22.0,
                map: MapId::Town,
                x: 18,
...
fn sam_schedule(season: Season) -> NpcSchedule {
    let (home_map, home_x, home_y) = (MapId::TownHouseEast, 9, 8);
    ...
    NpcSchedule {
        ...
        festival_override: Some(festival_override_for_season(
            season, home_map, home_x, home_y, 20, 18,
        )),
    }
}
...
fn mira_schedule(season: Season) -> NpcSchedule {
    let (home_map, home_x, home_y) = (MapId::TownHouseWest, 3, 9);
    ...
    NpcSchedule {
        ...
        rain_override: Some(vec![
            ...
            ScheduleEntry {
                ...
                x: 16,
                y: 14,
            }, // light rain work (rain is good!)
            ScheduleEntry {
                time: 12.0,
                map: MapId::Farm,
                x: 14,
                y: 14,
            }, // inside for heavy rain
            ScheduleEntry {
                time: 20.0,
                map: MapId::Farm,
                x: 14,
    ...
    NpcSchedule {
        weekday: vec![
            ...
            ScheduleEntry {
                ...
                x: 26,
                y: 18,
            }, // east plaza exploring
            ScheduleEntry {
                time: 16.0,
                map: MapId::Town,
                x: 22,
                y: 18,
            }, // afternoon in plaza (south of Blacksmith)
            ScheduleEntry {
                time: 18.0,
                map: MapId::Town,
                x: 10,
        ...
        weekend: vec![
            ...
            ScheduleEntry {
                time: 14.0,
                map: afternoon_map,
                x: aft_x.saturating_add(4),
                y: aft_y.saturating_add(4),
            }, // farther
            ScheduleEntry {
                time: 16.0,
                map: MapId::Town,
...
mod tests {
    ...
    fn test_schedules_have_non_empty_weekday_and_weekend() {
        ...
        for id in &npc_ids {
            ...
            assert!(
                !sched.weekday.is_empty(),
                "{} weekday should not be empty",
                id
            );
            assert!(
                !sched.weekend.is_empty(),
                "{} weekend should not be empty",
                id
            );
        }
    }
    ...
    fn test_schedules_have_rain_and_festival_overrides() {
        ...
        for id in &npc_ids {
            ...
            assert!(
                sched.rain_override.is_some(),
                "{} should have rain_override",
                id
            );
            assert!(
                sched.festival_override.is_some(),
                "{} should have festival_override",
                id
            );
        }
    }
