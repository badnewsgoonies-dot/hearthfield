# Sub-Agent Playbook — Project Instructions (v2: Contrastive-Causal Edition)

Use this as a procedural manual. Follow it in order. Do not skip steps.

**Mission:** Ship a working build with zero handwritten code by enforcing (1) a frozen type contract, (2) mechanical scope clamping, (3) compiler → tests gates, and (4) contrastive-causal worker specs that teach the correct path, the tempting wrong path, and the consequence boundary between them.
...
- blame-heavy prompts produce apologies, rigidity, scope creep, and framework-building
...
Done = both commands pass, no skipped tests, and the self-check is answered.
