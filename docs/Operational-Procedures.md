...
2. State the intended player-facing change in one sentence.
3. Implement.
...
2. **Clamp breaks the fix** — seam is wrong or task is integration work
3. **False green** — compile/tests pass but shared contract is not actually used
4. **Beautiful dead game** — structural gates green, reality gates red
5. **Ghost progress** — nothing new is player-reachable
6. **Search termination on green gates** — commit after Gate without Harden
